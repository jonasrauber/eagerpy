...
def l0(x: TensorType, axis: Optional[Axes] = None, keepdims=False) -> TensorType:
    return (x != 0).sum(axis=axis, keepdims=keepdims)


def l1(x: TensorType, axis: Optional[Axes] = None, keepdims=False) -> TensorType:
    return x.abs().sum(axis=axis, keepdims=keepdims)


def l2(x: TensorType, axis: Optional[Axes] = None, keepdims=False) -> TensorType:
    return x.square().sum(axis=axis, keepdims=keepdims).sqrt()


def linf(x: TensorType, axis: Optional[Axes] = None, keepdims=False) -> TensorType:
    return x.abs().max(axis=axis, keepdims=keepdims)


def lp(
    x: TensorType, p: Union[int, float], axis: Optional[Axes] = None, keepdims=False
) -> TensorType:
    if p == 0:
